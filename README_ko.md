...
- **세션 시간 측정** (v1.6.0): 자동 시간 추적 — 세션 시작, 랩업 시각, 소요 시간, 누적 협업 시간
...
- **세션 시작** — 세션 JSONL 파일 첫 엔트리의 타임스탬프에서 추출 (UTC → 로컬 변환)
- **랩업 시각** — `/wrapup` 호출 시점
- **소요 시간** — 시작~랩업 간 소요 시간 (`Xh Ym` 형식)
- **구간 분리** — 같은 세션에서 두 번째 랩업 시, 직전 랩업 시각부터 구간 측정
- **누적 통계** — 전체 프로젝트 합산 랩업 횟수 + 누적 협업 시간
