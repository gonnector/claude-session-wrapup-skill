...
**텍스트 피드백**: AskUserQuestion의 questions 배열로 3개 질문을 한 번에 표시.
- 각 질문의 options: "비워두기" (빈 배열로 설정) / "AI 추출" (대화 컨텍스트에서 추출 시도)
- "Other" 선택 시 직접 입력한 값을 사용
...
**직접 등록**: `/todo` Skill을 호출하지 않고 `register-todos.py`를 직접 실행. wrapup에서 이미 선택이 확정되었으므로, /todo의 추출→선택 UX를 다시 거치면 중복 인터랙션 발생.
