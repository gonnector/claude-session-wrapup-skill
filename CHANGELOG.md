...
### Changed
- **Step 5 ToDo 등록 UX 개선** — AskUserQuestion 4개 옵션 제한으로 항목 누락되던 문제 해결
  - 액션 아이템 + 추천 작업을 순차 번호(1, 2, 3, ...)로 통합 표시
  - 선택지: 전부 등록 / 액션 아이템만 / 추천 작업만 / 나중에
  - "Other"로 번호 직접 입력 가능 (예: `1,3,5`)
  - 채택/도출 카운트 추적 → Step 9에서 표시
- **Step 9 완료 메시지 전면 개편**
  - 타이틀: `## [랩업 완료] {session_name}`
  - 섹션 제목: `**▸ 제목**` (볼드 + 기호) + 내용 들여쓰기
  - `저장됨` → `학습/기억` (세션 요약 경로 제거, 3항목 나란히 배치)
  - `연계 아이디어` → `추천 아이디어`
  - ToDo 섹션 신설 (세션 평가 위): 채택/도출 현황 표시
  - 누적 통계 3줄 재구성: 세션·랩업·협업 / 학습 / ToDo 상태별

### Added
- **고유 세션 수 추적** (`unique_sessions`): collect-meta.py에서 전체 summaries의 고유 session_id 수 집계
- **ToDo 상태별 통계**: Step 9에서 todo.xlsx 직접 조회하여 활성/진행/대기 건수 및 비율 표시
